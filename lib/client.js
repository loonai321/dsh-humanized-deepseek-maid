...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const okStyle = { color: '#3fb36a', fontSize: 12, marginTop: 6 };
    ...
    function makeCard() {
      function MaidCard() {
        ...
        return h('div', { style: cardStyle },
          ...
          saved ? h('div', { style: okStyle }, '✓ 保存成功（已立即生效' + (status && typeof status.configRevision === 'number' ? '，配置版本 ' + status.configRevision : '') + '）') : null,
    ...
    exports.apply = function apply(ctx) {
      ...
      ctx.slots.inject('settings.plugin.item', () => slots.register({
        ...
        key: 'dsh-humanized-maid-settings',
